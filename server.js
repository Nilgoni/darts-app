...
<head>
  ...
  <title>Darts App – Spieltag</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body class="bg-light">
  <div class="container py-5">
    <h1 class="text-center mb-5">🎯 Spieltag</h1>
    ...
    <div id="closeButton" class="text-center mb-4"></div>
  ...
  <script>
    ...
    async function loadSpieltag() {
      try {
        ...
        spieltagData = await res.json();

        document.getElementById('info').innerHTML = `
          <p><strong>Modus:</strong> ${spieltagData.spieltag.modus === 'best_of' ? 'Best of' : 'First to'} ${spieltagData.spieltag.runden}</p>
          <p><strong>Rundentyp:</strong> ${spieltagData.spieltag.rundenTyp === 'hin' ? 'Nur Hinrunde' : 'Hin- und Rückrunde'}</p>
          <p><strong>Teilnehmer:</strong> ${spieltagData.spieltag.teilnehmer.map(u => u.username).join(', ')}</p>
          <p><strong>Status:</strong> ${spieltagData.spieltag.abgeschlossen ? 'Beendet' : 'Offen'}</p>
        `;

        const closeBtnDiv = document.getElementById('closeButton');
        if (!spieltagData.spieltag.abgeschlossen) {
          closeBtnDiv.innerHTML = `
            <button class="btn btn-warning btn-lg" onclick="closeSpieltag()">Spieltag beenden</button>
          `;
        } else {
          closeBtnDiv.innerHTML = '<p class="text-muted">Dieser Spieltag ist beendet und nicht mehr bearbeitbar.</p>';
        }

        const matchesDiv = document.getElementById('matches');
        matchesDiv.innerHTML = '';

        spieltagData.matches.forEach((match, index) => {
          ...
          div.innerHTML = `
            <div class="card-body">
              ...
              ${spieltagData.spieltag.abgeschlossen || match.abgeschlossen ? `
    ...
    async function closeSpieltag() {
      if (!confirm('Spieltag wirklich beenden? Danach können keine Ergebnisse mehr geändert werden.')) return;
    ...
    async function loadTabelle() {
      try {
        ...
        document.getElementById('currentTable').innerHTML = '<p class="text-danger">Fehler</p>';
        document.getElementById('overallTable').innerHTML = '<p class="text-danger">Fehler</p>';
      }
    }
